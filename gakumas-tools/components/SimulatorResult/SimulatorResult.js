...
const DEFAULT_TABS = ["stats", "logs", "compare"];
const TAB_STORAGE_KEY = "simulatorResultTab";

function SimulatorResult({ data, config, enterPercents, listenerData, idolId, plan }) {
  ...
  return (
    <div id="simulator_result" className={styles.result}>
      ...
      <div className={styles.details} data-export-hide="true">
        ...
        {tab === "useStats" && listenerData["UseStats"] && (
          <SimulatorUseStats
            useStats={listenerData["UseStats"]}
            idolId={idolId}
          />
        )}
        {tab === "conditionalUseStats" && listenerData["ConditionalUseStats"] && (
          <SimulatorConditionalUseStats
            conditionalUseStats={listenerData["ConditionalUseStats"]}
            idolId={idolId}
          />
        )}
        {tab === "priorityStats" && listenerData["PriorityStats"] && (
          <SimulatorPriorityStats
            priorityStats={listenerData["PriorityStats"]}
            idolId={idolId}
          />
        )}
        {tab === "scoreStats" && listenerData["ScoreStats"] && (
          <SimulatorScoreStats
